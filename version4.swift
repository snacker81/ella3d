...
// ============================================================
// AUDIO - ALL SOUNDS ARE GENERATED IN MEMORY
// ============================================================
...
final class GameAudio {
    ...
    private let shotPlayers = (0..<4).map { _ in AVAudioPlayerNode() }
    private let explosionPlayers = (0..<4).map { _ in AVAudioPlayerNode() }
    ...
    private var nextExplosion = 0
    private var isRunning = false

    private lazy var musicBuffer = makeMusic()
    private lazy var shotBuffer = makeShot()
    private lazy var explosionBuffer = makeExplosion()

    init() {
        for player in [music] + shotPlayers + explosionPlayers {
            engine.attach(player)
            engine.connect(player, to: engine.mainMixerNode, format: format)
        }

        music.volume = 0.30
        shotPlayers.forEach { $0.volume = 0.55 }
        explosionPlayers.forEach { $0.volume = 0.75 }
    }
    ...
    func start() {
        ...
        do {
            ...
            // The game remains playable if audio output is unavailable.
            NSLog("Could not start game audio: %@", error.localizedDescription)
        }
    }
    ...
    func playShot() {
        ...
        let player = shotPlayers[nextShot]
        nextShot = (nextShot + 1) % shotPlayers.count
        player.stop()
        player.scheduleBuffer(shotBuffer, at: nil)
        player.play()
    }
    ...
    func playExplosion() {
        ...
        let player = explosionPlayers[nextExplosion]
        nextExplosion = (nextExplosion + 1) % explosionPlayers.count
        player.stop()
        player.scheduleBuffer(explosionBuffer, at: nil)
        player.play()
    }
    ...
    private func buffer(from samples: [Float]) -> AVAudioPCMBuffer {
        ...
        samples.withUnsafeBufferPointer { source in
            result.floatChannelData![0].update(
    ...
    private func makeMusic() -> AVAudioPCMBuffer {
        let stepSeconds = 0.25
        let stepCount = 32
        let stepFrames = Int(sampleRate * stepSeconds)
        var samples = [Float](repeating: 0, count: stepFrames * stepCount)

        // A short, looping 8-bit tune: melody, bass, and a quiet ticking beat.
        let melody = [
            76, 0, 79, 0, 83, 81, 79, 0,
            74, 0, 79, 0, 81, 79, 76, 0,
            72, 0, 76, 0, 79, 76, 74, 0,
            74, 76, 79, 0, 76, 74, 72, 0
        ]
        let bass = [40, 40, 43, 43, 36, 36, 38, 38]

        func addNote(_ note: Int, at start: Int, length: Int, volume: Double, pulse: Bool) {
            let frequency = 440.0 * pow(2.0, Double(note - 69) / 12.0)
            let count = min(length, samples.count - start)
            ...
            for offset in 0..<count {
                ...
                let wave = pulse
                    ? (phase < 0.25 ? 1.0 : -1.0)
                    : (1.0 - 4.0 * abs(phase - 0.5))
                let attack = min(1.0, time / 0.008)
                let release = min(1.0, Double(count - offset) / (sampleRate * 0.06))
                samples[start + offset] += Float(wave * volume * attack * release)
            }
        }
        ...
        for step in 0..<stepCount {
            ...
            if melody[step] != 0 {
                addNote(melody[step], at: start, length: stepFrames, volume: 0.16, pulse: true)
            }

            if step.isMultiple(of: 4) {
                addNote(bass[step / 4], at: start, length: stepFrames * 3, volume: 0.19, pulse: false)
            }

            // A soft, deterministic hi-hat keeps time without an audio file.
            var noiseState = UInt32(step + 1) &* 747_796_405
            let hatFrames = Int(sampleRate * 0.035)

            for offset in 0..<hatFrames {
                noiseState = noiseState &* 1_664_525 &+ 1_013_904_223
                let noise = Double(noiseState >> 16) / 32_768.0 - 1.0
                let fade = 1.0 - Double(offset) / Double(hatFrames)
                samples[start + offset] += Float(noise * fade * 0.045)
            }
        }
    ...
    private func makeShot() -> AVAudioPCMBuffer {
        let count = Int(sampleRate * 0.19)
        var samples = [Float](repeating: 0, count: count)
        var noiseState: UInt32 = 123_456_789
        var phase = 0.0

        for index in samples.indices {
            let time = Double(index) / sampleRate
            let progress = Double(index) / Double(count)
            let frequency = 1_250.0 * pow(0.18, progress)
            phase += frequency / sampleRate
            let laser = sin(2 * .pi * phase)
            noiseState = noiseState &* 1_664_525 &+ 1_013_904_223
            let noise = Double(noiseState >> 16) / 32_768.0 - 1.0
            let envelope = exp(-time * 20.0)
            samples[index] = Float((laser * 0.75 + noise * 0.25) * envelope * 0.8)
        }

        return buffer(from: samples)
    }

    private func makeExplosion() -> AVAudioPCMBuffer {
        let count = Int(sampleRate * 0.55)
        var samples = [Float](repeating: 0, count: count)
        var noiseState: UInt32 = 987_654_321
        var phase = 0.0
        var lowNoise = 0.0
        ...
        for index in samples.indices {
            let time = Double(index) / sampleRate
            let progress = Double(index) / Double(count)
            let frequency = 160.0 * pow(0.25, progress)
            phase += frequency / sampleRate
            noiseState = noiseState &* 1_664_525 &+ 1_013_904_223
            let noise = Double(noiseState >> 16) / 32_768.0 - 1.0
            lowNoise += (noise - lowNoise) * 0.12
            let rumble = sin(2 * .pi * phase)
            let envelope = exp(-time * 6.0)
            samples[index] = Float((rumble * 0.45 + lowNoise * 0.8 + noise * 0.12) * envelope)
        }
